...
function showPollForm($vars){
    ...
    <div id="show-form<?php echo $poll->id;?>" class="show-form<?php echo $poll->id;?>">
        <?php
        print '<table style="border: none !important;">';
        foreach($poll_answers as $answer){
            if($poll->answer_type == "one"){
                    print '<tr style="border: none !important;"><td style="border: none !important;" width="20" valign="middle"><input type="radio" name="'.$poll->id.'" value="'.$answer->id.'" /></td>';
                    if($poll->poll_type == 'image_poll') {
                        print '<td style="border: none !important;"><img src="'.$answer->answer.'" width="100" alt="'.$answer->answer.'" /></td>';
                    }
                    else {
                        print '<td style="border: none !important;" align="left">'.$answer->answer.'</td>';
                    }
                    print '</tr>';
            }
            if($poll->answer_type == "multiple"){
                print '<tr style="border: none !important;"><td style="border: none !important;" width="20" valign="middle"><input type="checkbox" name="option'.$option.'" value="'.$answer->id.'" /></td>';
                if($poll->poll_type == 'image_poll') {
                        print '<td style="border: none !important;"><img src="'.$answer->answer.'" width="100" alt="'.$answer->answer.'"/></td>';
                    }
                    else {
...
function showPollFormSC($vars){
    ...
    <div id="show-form<?php echo $poll->id;?>" class="show-form<?php echo $poll->id;?>">
        ...
        foreach($poll_answers as $answer){
            if($poll->answer_type == "one"){
                    print '<tr style="border: none !important;"><td valign="middle"><input type="radio" name="'.$poll->id.'" value="'.$answer->id.'" /></td>';
                    if($poll->poll_type == 'image_poll') {
                        print '<td style="border: none !important;"><img src="'.$answer->answer.'" width="100" alt="'.$answer->answer.'" /></td>';
                    }
                    else {
                        print '<td style="border: none !important;">'.$answer->answer.'</td>';
                    }
                    print '</tr>';
            }
            if($poll->answer_type == "multiple"){
                print '<tr style="border: none !important;"><td style="border: none !important;" valign="middle"><input type="checkbox" name="option'.$option.'" value="'.$answer->id.'" /></td>';
                if($poll->poll_type == 'image_poll') {
                        print '<td style="border: none !important;"><img src="'.$answer->answer.'" width="100"  alt="'.$answer->answer.'" /></td>';
                    }
                    else {
